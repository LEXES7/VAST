...
export function DealStageSelect({ id, stage }: { id: string; stage: string }) {
  ...
  return (
    <select
      ...
      onChange={onChange}
      disabled={busy}
      className="mt-2 w-full rounded-lg border border-white/10 bg-white/[0.03] px-2 py-1 text-xs text-white/60 transition hover:border-white/25 focus:border-ember-500/60 focus:outline-none disabled:opacity-50"
    >
      {DEAL_STAGES.map((s) => (
